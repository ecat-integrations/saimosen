 ...
 * 的 round 入口——负向窗内「至多一个」入口=链已停。快节拍（150ms）下旧「阈值-2 全程计数」
 * 形态不可用：sweep 前的正常周期同样计数、与真 stray 无法区分，改为 sweep 后武装的
 * 定向计数窗（300ms 窗 ≥ 2 拍×150ms，等价原「6s 窗 > 5s 生产周期」覆盖强度）。
 * 锚在轮询入口（executePolling 每轮首访 acquirePollingBounded）而非块读：在飞轮次的迟到续块
 ...
 * shot 出队→acquirePollingBounded 之间被抢占，恢复后会在武装<b>之后</b>迟到触入口。该迟到入口
 * 是合法在飞轮（发起于 stop 前）不是新轮，容 1 个；真「链未停」形态按 150ms 节拍在
 * 300ms 窗内 ≥2 次入口，2 阈值立即红，检出力不变。
 *
 * <p>acquirePollingBounded 首访放行真轮、其后以 null 完成（锁忙弃轮不触源读，SDK 内部消化）——
 * verify(times(1)) 块参数断言对 150ms 快节拍保持确定性（无第二轮真实读竞争）。
...
final class RoundEntryProbe {
    ...
    /** 挂到 mock 源的 acquirePollingBounded 入口（须在轮询 start 前一次完成改桩）。 */
    static RoundEntryProbe on(ModbusSource mockSource) {
        RoundEntryProbe probe = new RoundEntryProbe();
        when(mockSource.acquirePollingBounded(anyLong())).thenAnswer(inv -> {
            probe.onRoundEntry();
            return probe.grant();
        });
        return probe;
    ...
    private CompletableFuture<String> grant() {
        return granted.compareAndSet(false, true)
                ? CompletableFuture.completedFuture("testKey")
