 ...
 * 的 round 入口——负向窗内零计数=零新轮。快节拍（50ms）下旧「阈值-2 全程计数」形态
 * 不可用：sweep 前的正常周期同样计数、与真 stray 无法区分，改为 sweep 后武装的定向
 * 计数窗（600ms 窗覆盖 >10 个 50ms 周期，等价原「6s 窗 > 5s 生产周期」的覆盖强度）。
...
final class RoundEntryProbe {
    ...
    final CountDownLatch strayRound = new CountDownLatch(1);
